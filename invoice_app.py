...
if not st.session_state.logged_in:
    ...
    with tab1:
        ...
        remember = st.checkbox("Keep me logged in") # MEMORY FEATURE
        if st.button("Login"):
            user = get_user(email)
            if user and user[1] == password:
                ...
                if remember:
                    st.session_state.remember_email = email # saves in session
    ...
    with tab2:
        ...
        if st.button("Create Account"):
            if create_user(email, password, business):
                ...
                st.error("This email already has an account. Please Login")
    ...
    customer = st.text_input("Customer Name / WhatsApp Number e.g 2348012345678")
